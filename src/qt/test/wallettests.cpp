...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<TangerineAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_tangerine-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_tangerine-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_tangerine-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = TangerineUnits::formatWithUnit(unit, balance, false, TangerineUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    TangerineAmountField* amountInput = receiveCoinsDialog.findChild<TangerineAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("tangerine:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_tangerine-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
