...
#include <qt/tangerineaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tangerine address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TangerineAddressEntryValidator(parent));
    widget->setCheckValidator(new TangerineAddressCheckValidator(parent));
}

bool parseTangerineURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tangerine: URI
    if(!uri.isValid() || uri.scheme() != QString("tangerine"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TangerineUnits::parse(TangerineUnits::TNG, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTangerineURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTangerineURI(uriInstance, out);
}

QString formatTangerineURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tangerine:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TangerineUnits::format(TangerineUnits::TNG, info.amount, false, TangerineUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTangerineConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", TANGERINE_CONF_FILENAME));
    ...
    /* Open tangerine.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tangerine.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tangerine (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Tangerine (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Tangerine*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "tangerine.desktop";
    return GetAutostartDir() / strprintf("tangerine-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a tangerine.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Tangerine\n";
        else
            optionFile << strprintf("Name=Tangerine (%s)\n", chain);
