...
const char * const TANGERINE_CONF_FILENAME = "tangerine.conf";
const char * const TANGERINE_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/tangerine/tangerine/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //tangerine-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", TANGERINE_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "tangerine";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Tangerine
    // macOS: ~/Library/Application Support/Tangerine
    // Unix-like: ~/.tangerine
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tangerine";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Tangerine";
#else
    // Unix-like
    return pathRet / ".tangerine";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", TANGERINE_CONF_FILENAME);
