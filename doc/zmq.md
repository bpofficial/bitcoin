...
The Tangerine Core daemon can be configured to act as a trusted "border
router", implementing the tangerine wire protocol and relay, making
...
The ZeroMQ feature in Tangerine Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building tangerined:
...
For instance:
    ...
    $ tangerined -zmqpubhashtx=tcp://127.0.0.1:25112 \
               -zmqpubhashtx=tcp://192.168.1.2:25112 \
               -zmqpubrawtx=ipc:///tmp/tangerined.tx.raw \
...
These options can also be provided in tangerine.conf.
...
From the perspective of tangerined, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tangerined directly. Furthermore, no information is
...
using. Tangerined appends an up-counting sequence number to each
