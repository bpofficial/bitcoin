...
This guide describes how to build tangerined, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Tangerine Core from source.
...
### 4. Clone Tangerine repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Tangerine Core repository to a directory.
...
It is not necessary to build wallet functionality to run `tangerined` or  `tangerine-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/tangerine/tangerine/blob/master/doc/descriptors.md), `sqlite` is required.
...
Tangerine Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/tangerine/tangerine/issues/7714).
...
You can deploy a `.dmg` containing the Tangerine Core application using `make deploy`.
...
## Building Tangerine Core

### 1. Configuration

There are many ways to configure Tangerine Core, here are a few common examples:
...
Run the following in your terminal to compile Tangerine Core:
...
## Running Tangerine Core

Tangerine Core should now be available at `./src/tangerined`.
If you compiled support for the GUI, it should be available at `./src/qt/tangerine-qt`.

The first time you run `tangerined` or `tangerine-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Tangerine/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Tangerine"

touch "/Users/${USER}/Library/Application Support/Tangerine/tangerine.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Tangerine/tangerine.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Tangerine/debug.log
```

## Other commands:

```shell
./src/tangerined -daemon      # Starts the tangerine daemon.
./src/tangerine-cli --help    # Outputs a list of command-line options.
./src/tangerine-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/tangerine-qt -server # Starts the tangerine-qt server mode, allows tangerine-cli control
```
