# Fuzzing Tangerine Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Tangerine Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/tangerine/tangerine
$ cd tangerine/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/tangerine/tangerine/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/tangerine/tangerine/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/tangerine/tangerine/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/tangerine/tangerine/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`tangerine-core/qa-assets`](https://github.com/tangerine-core/qa-assets) repo.

To fuzz `process_message` using the [`tangerine-core/qa-assets`](https://github.com/tangerine-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/tangerine-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`tangerine-core/qa-assets`](https://github.com/tangerine-core/qa-assets) repo.

Every single pull request submitted against the Tangerine Core repo is automatically tested against all inputs in the [`tangerine-core/qa-assets`](https://github.com/tangerine-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Tangerine Core more robust.
...
with certain assembly code from Tangerine Core's code. See [developer notes on sanitizers](https://github.com/tangerine/tangerine/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Tangerine Core using afl++

## Quickstart guide

To quickly get started fuzzing Tangerine Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Tangerine Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Tangerine Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Tangerine Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Tangerine
Core without having to write any custom fuzzing harness. The `tangerined` server
...
```sh
$ mkdir tangerine-honggfuzz-p2p/
$ cd tangerine-honggfuzz-p2p/
$ git clone https://github.com/tangerine/tangerine
$ cd tangerine/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/tangerined.cpp b/src/tangerined.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/tangerined.cpp
+++ b/src/tangerined.cpp
...
Tangerine Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/tangerine-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=tangerine-core).
...
because Tangerine is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_tangerine-core/latest).
