...
Then, to tunnel a SSL connection on 25112 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 25112 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:5112/
        ProxyPassReverse http://127.0.0.1:5112/
...
- #7209 `f3ad812` test: don't override TANGERINED and TANGERINECLI if they're set (Wladimir J. van der Laan)
...
- #6401 `6db53b4` Add TANGERINED_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
...
- #6504 `b6fee6b` Rationalize currency unit to "TNG" (Ross Nicoll)
